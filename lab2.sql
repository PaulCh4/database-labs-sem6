...
(3)
\\
КВЕСТ 1: у вас процедуры сранвиваются?
\\
создайте процедуры 2/ с одним названием но с разным кодом
сравнивать по содержимому функций а не по параметрам
\
\
\
КВЕСТ 2: 
    сравнение индексов в триггерах
    измените мне компорейшн ???
    айдишники двух лаб 
    
    ГДЕ DDL СКРИПТ ДЛЯ СОЗДАНИЯ\УДАЛЕНИЯ ---- КОД КОТОРЫЙ ВЫВЕДЕТ ВСЕ ОТЛИЧИЯ И КАК ИХ УРОВНЯТЬ
\
\
\
\
КВЕСТ 3:
\\
 ЗАКАЛЬЦОВЫВАЮТСЯ ЛИ ТАБЛИЦЫ
 
 создайте 3 таблицы tabl1 2 3 о в любой схеме от любого юзера --- в девелопер схеме 
 без fk 
 2->1
 2->3
...
(4)
читать что-то из файла
\\
место где вы парсите конкретный ворпос
dml
ddl
select
распределение
...
(к) 20 вопросов ---- 8\20 ответов -> 4
- тест
- сущность изображается на uml будет в виде ---- прямоугольник
- за что отвечает комит ---- потдверждение транзакции 
- что такое системный кеш блоков plsql ---- SGA
- 
- 
- 
- 
